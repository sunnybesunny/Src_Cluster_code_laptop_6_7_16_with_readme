...
def Pmut(p):
    # 8 bins, normalized, random assignment of delX among 8bins

    if p<=0.015:
        rd=-6.5+method.rand_gen()
    elif p<=0.055:
        rd=-5.5+method.rand_gen()
    elif p<=0.135:
        rd=-4.5+method.rand_gen()
    elif p<=0.225:
        rd=-3.5+method.rand_gen()
    elif p<=0.375:
        rd=-2.5+method.rand_gen()
    elif p<=0.6:
        rd=-1.5+method.rand_gen()
    elif p<=0.95:
        rd=-0.5+method.rand_gen()
    else:
        rd= 0.5+method.rand_gen()

    return (rd+method.sht)    # Energy value + shift, method.sht:0
...
#                     method.n[l]+=1
#
#     E_sel_Hist.write("# l(Bin.#)\t E_l(Eny.Range)\t method.n[l](#sel.B)\t method.P[l](sel.B.ratioinBin)\n")
#
#     for l in range(0, method.Bin_E_sel): #fprint histogram of all BCs vs energy
#         method.P[l]=float(method.n[l])/float(method.n_sel)
#         E_sel_Hist.write("%d %.3f %d %.3f\n" % (l, E_min+(l+0.5)*delta_E, method.n[l], method.P[l]))
#
#     E_sel_Hist.write("\n method.E_sel_min=%.3f, method.E_sel_max=%.3f, del_E=%.3f\n" % (E_min, E_max, delta_E))
